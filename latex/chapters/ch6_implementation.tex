...
\begin{table}[htp!]
    ...
    \begin{tabularx}{\linewidth}{lXc}
        ...
        \texttt{GET /lectures}                & Gets all lectures a student is enrolled or waitlisted in & 200               \\
        \addlinespace
        \texttt{POST /courses/create}         & Creates a course                                         & 201               \\
        \addlinespace
        \texttt{POST /lectures/\{id\}/enroll} & Enroll to a lecture                                      & 201               \\
        \addlinespace
        \texttt{GET /stats/credits}           & Gets a student's current credits                         & 201               \\
        \addlinespace
        \texttt{GET /stats/grades/history}    & Gets the grade history for an assessment                 & 201               \\
        \bottomrule
    \end{tabularx}
...
TODO Show command and query gateway and illustrate example flow through an Axon application? -> refer to "Tracing request flow"
...
Figure \ref{fig:crud-er-diagram} shows the Entity Relationship Diagram for the CRUD app. It includes nine entities and a value object for the app's relational database schema. It should be noted that the auditing table created by Hibernate Envers are not present in the diagram. The diagram highlights that \keyw{LectureEntity} is the core entity in the system, having direct a relationship to many other entities.
...
Following~\cite{jogalekar_evaluating_2000}, a function to determine the scalability of applications is established in this subsection. Their scalability metric, described in \autoref{sec:quantifying-scalability-metric}, serves as a basis for the scalability metric used in this study.

This cost function is used:

\[C(k) = w_1 * \text{CPU} + w_2 * \text{S} + w_3 \left( \frac{T}{200} \right) ^2 + w_4 \left( \frac{D}{10} \right)^2\]

The cost function is weighted, with $w_i$ being the weights for each metric. $S$ corresponds to total storage size. $T$ corresponds to $tomcat\_threads$, $D$ corresponds to $hikari\_connections$. These metrics are normalized to their respective ceilings.
...
Using the above cost function $C(k)$, the productivity and scalability of the applications can be calculated as described in \autoref{sec:quantifying-scalability-metric}, with throughput defined as $RPS - dropped\_iterations\_rate$ and the value function defined as $f(k) = \frac{1}{latency\_p95}$. The implementation of the scalability function is available in \javaname{scalability\_function.py}\footnote{\javaname{performance-tests/scalability\_function.py}}.
...
Other metrics outlined in \autoref{sec:flexibility-architectural-metrics} are also supported by MetricsReloaded, such as \gls{mood}, \gls{ck} and the stability metrics described in~\cite{martin_agile_2003}. It should be noted that the \gls{mood} metrics are only partly implemented, missing calculations for \gls{clf} and \gls{rf}, which is why no results will be presented for those metrics.
