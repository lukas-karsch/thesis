...
\autoref{table:endpoints} presents a feature matrix, mapping \acrshort{http} endpoints to their functionality. As this thesis does not focus on the functionality of an application, but rather on an architectural comparison, only the endpoints which will later be load-tested are listed.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{lXc}
        ...
        \addlinespace
        \texttt{POST /lectures/create}        & Creates a lecture                                           & 201               \\
        \addlinespace
        \texttt{GET /lectures/\{id\}}         & Gets a lecture's details by ID                              & 200               \\
        \addlinespace
...
\gls{jpa}\footnote{\url{https://jakarta.ee/specifications/persistence/}}, formerly Java Persistence \gls{api} is a Java specification which provides a mechanism for managing persistence and object-relational mapping. \glspl{orm} act as a mapper between the relational world of SQL databases and the object-oriented world of Java~\cite[Chapter~1]{bauer_java_2016}.
...
Envers\footnote{\url{https://hibernate.org/orm/envers/}} is a Hibernate project designed for audit logging and versioning of historic data. When using Envers, a copy of data (a "revision") is stored in specific revision tables managed by Envers~\cite[Chapter~13.3]{bauer_java_2016}. Envers offers an API, primarily accessed through the \keyw{AuditReader}, to query historic versions of data~\cite[Chapter~13.3]{bauer_java_2016}.
...
Axon Server\footnote{\url{https://www.axoniq.io/server}} is a platform designed specifically for Event-driven systems. It functions as both a high-performance Event Store and a dedicated Message Router for commands, queries, and events. By bundling these responsibilities into a single service, Axon Server replaces the need for separate infrastructures such as a relational database for events and a message broker like Kafka\footnote{\url{https://kafka.apache.org/}} or RabbitMQ\footnote{\url{https://www.rabbitmq.com/}} for communication. Axon Server is designed to seamlessly integrate with Axon Framework. When using the Axon Server Connector, the application automatically finds and connects to the Axon Server. It is then possible to use Axon server without further configuration~\cite{axoniq_introduction_2025-1,axoniq_axon_2025}.
...
Subscription queries provide an initial result and continuous updates as data changes. These queries work with reactive programming~\cite{axoniq_query_2025}. Through the use of subscription queries in a web controller, a synchronous user experience can be achieved while embracing Axon's asynchronous internals, by waiting for the result of a command to be reflected in a read model and then returning that updated value to the client.
...
In practice, this validation logic can be encapsulated within a domain service or a validator interface that is injected directly into command handlers. The validator interacts with the lookup table repository to verify global invariants before the Aggregate state is modified. By separating the lookup logic from the read-model, the system avoids the latency of eventual consistency while maintaining the architectural integrity of the Aggregate as a boundary for consistency. This pattern helps to enforce validation across individual aggregates and allows for global state verification~\cite{ceelie_set_2020}.
...
Actuator can be enabled in a SpringBoot project by including the \javaname{spring-boot-starter-actuator} dependency~\cite{broadcom_inc_enabling_2026}.
...
\subsection{Mapping Technologies to Requirements}

These technologies are used applied to implement the requirements defined in \autoref{ch:requirement-analysis}. \autoref{tab:requirements-to-technology} presents a mapping of those requirements to each used technology.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{llX}
        \toprule
        \textbf{Requirement} & \textbf{Technologies}          & \textbf{Role}                                                                                                    \\
        \midrule
        Public Interface     & SpringBoot, Jackson            & SpringBoot provides the Web interface using Jackson for \gls{dto} serialization.                                 \\
        \addlinespace
        Entities             & PostgreSQL, JPA, Axon, Jackson & Used to model data and entities.                                                                                 \\
        \addlinespace
        Business Rules       & SpringBoot, Axon               & SpringBoot and Axon provide the framework to handle requests and enforce business rules.                         \\
        \addlinespace
        \glspl{slo}          & k6                             & Develop and run load tests to validate \glspl{slo}.                                                              \\
        \addlinespace
        Auditing             & Envers, Axon                   & In CRUD, Envers provides an audit log. In ES-CQRS, Axon's event log serves as an audit trail.                    \\
        \addlinespace
        Observability        & Actuator, Prometheus           & Actuator provides metric endpoints that can be scraped by Prometheus, which then presents the data for querying. \\
        \addlinespace
        Contract             & JUnit, REST Assured            & Tests enforce an identical contract for both implementations.                                                    \\
        \addlinespace
        Environment          & Docker                         & Used to containerize the implementations and provide a controlled environment.                                   \\
        \bottomrule
    \end{tabularx}
    \caption{Mapping of requirements to technologies}
    \label{tab:requirements-to-technology}
\end{table}
...
Like the \acrshort{crud} application, this prototype also implements Controller interfaces defined by the shared \texttt{api} module. However, each feature slice contains its own \texttt{api} package that is shared between web controllers, command-side and read-side. This "internal" \gls{api} maps the application's public interface to \gls{cqrs} semantics by defining specific command and query classes which target command and query Handlers. These feature-specific \texttt{api} packages are the only public packages in a feature slice, meaning communication across package boundaries is only possible using the defined \gls{api} classes.
...
As described in \autoref{sec:event-sourcing}, no additional audit log has to be implemented when using \acrlong{es}. The application's state can be reconstructed using the Event Stream by rehydrating Aggregates (write-side) and by replaying projections (read-side), if desired.
...
The project's infrastructure is designed for consistency and reproducibility across development and testing environments. It is composed of a containerized environment for running the applications and their dependencies, an automated \gls{vm} provisioning setup for load testing, as well as an integration testing strategy using \glspl{testcontainer}, described in \autoref{sec:contract-test-implementation}.
...
Both implementations are containerized using \glspl{dockerfile}. Both use \keyw{amazoncorretto:25} as the base image, and the compiled Java application is copied into the container to be executed.
