...
\subsection{Graphs (TODO)}

In graphs that show latencies, the shaded areas in the line graphs represent the \emph{confidence interval (CI)} of the latency measurements. TODO: explain CI.

Graphs showing resource consumption typically show the area between the 25th and 75th percentile in the shaded area, also called \emph{\gls{iqr}}. TODO: does this make sense or should it also be CI?
...
The endpoint \texttt{POST /lectures/\{lectureId\}/enroll} enrolls students to a lecture. Its performance under load up to 500 \gls{rps} is visualized in \autoref{fig:l3_combined} using logarithmic x- and y-axes.
...
Size of the data store is presented in \autoref{fig:l3_data_size}.
...
\subsubsection{L4: Read Lectures (TODO resource, data size)}
\label{sec:l4}
...
In the client-side graph (\autoref{fig:l4_latency_vs_load_client}), both applications maintain $latency\_p50$ and $latency\_p95$ of below 10ms up to 3000 \gls{rps}. At 3000 \gls{rps}, the ES-CQRS application exhibits an around 6x higher $latency\_p95$ than the CRUD app. Beyond 3000 \gls{rps}, though, the CRUD latencies overtake the ES-CQRS latencies. At 4000 \gls{rps}, the ES-CQRS application exhibits a $latency\_p95$ of around 370ms, which is around 1.7x faster than  the CRUD application at 620ms and a $latency\_p50$ of 20ms, around a 6x speedup compared to CRUD.

Both applications violate \ref{slo-latency} beyond 3000 \gls{rps}.
...
Next, present resource consumption.
...
\subsubsection{L5: Get credits (TODO resource, data size)}
\label{sec:l5}
...
\subsubsection{L6: Create Lecture, then Read (TODO resource, data size)}
