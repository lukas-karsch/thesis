...
This final chapter discusses the findings by first identifying system bottlenecks and addressing anomalies observed during load testing. It provides an interpretation of the performance, architectural flexibility, and traceability results in the context of the established research questions. Finally, the research questions are answered, before outlining potential optimizations and directions for future work.
...
\hyperref[sec:l5]{Section \ref*{sec:l5}} (\gls{l}5) revealed inconsistencies in the collected resource consumption metrics for the CRUD application. Beyond 250 RPS, Prometheus could not detect any CPU usage, and the threadpool metrics exhibited a sharp decline in usage at 200 RPS and 250 RPS. As previously mentioned, these results were confirmed through repeated test executions, suggesting a consistent failure at this load level. A definitive root cause is difficult to pinpoint. One possible cause is resource contention within the shared infrastructure, as both the SpringBoot application and the PostgreSQL instance compete for CPU cycles on the same \gls{vm}. The computational intensity of the complex joins required for this endpoint may saturate the database, consequently starving the SpringBoot server of CPU time. Because a system call is necessary to determine the CPU usage of a process, metrics may not have been recorded accurately (or at all) due to high resource contention. However, this does not explain why the threadpool usage increases again at even higher loads. Ultimately, this shared-resource setup limits both performance of the applications and the effectiveness of the testing setup, making it difficult to identify the reason for such anomalies.

Furthermore, it is worth noting that in some cases, the metrics for data store size show an unexpected \gls{CI} in the ES-CQRS application. Several read-only tests show a non-zero \gls{CI}, which is surprising as the seed data is identical for each test and no further data is written to the system during load generation. This observation can be made in \gls{l}4, \gls{l}5, \gls{l}6, \gls{l}8. Identifying a reason for this effect would require further investigation.
...
Several tests show a decrease in storage size for the ES-CQRS application under increased load, which was attributed to projection lag. The projection lag is a very likely explanation, however no metrics were taken during the load tests which can definitely confirm this as the root cause. Furthermore, even projection lag does not fully explain why the storage taken at higher load is \emph{lower} than at a previous load. The reason for this observation is likely the fact that both write- and read-side utilize the same database connection pool. The write-side uses the database to write to the synchronous lookup tables, while the read-side attempts to use the same connections to keep its projections up to date. This issue is discussed further in \autoref{sec:limitations-of-the-study}.
...
The preservation of business intent further distinguishes the two architectures. While a CRUD audit log can capture intent by manually attaching context to transactions, this process is secondary to the state change. In contrast, events in an ES-CQRS architecture inherently communicate user intent because the event itself represents a specific business action rather than a generic data update. Through this, ES-CQRS ensures that the historical record remains semantically rich without requiring additional implementation for historic context.
...
\subsection{Conclusion: an Answer to the Main Research Question}

\begin{quote}
    \textit{How does an Event-sourced CQRS architecture compare to CRUD systems with an independent audit log regarding performance, scalability, flexibility and traceability?}
\end{quote}
...
\begin{table}[h!]
    ...
    \begin{tabularx}{\linewidth}{lXX}
        \toprule
        \textbf{Keyword}          & \textbf{Action}                                                                                                                                                                    & \textbf{Goal}                                                                                               \\
        \midrule
