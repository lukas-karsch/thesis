...
\textcite[264]{abbott_art_2009} mentions another way of looking at load testing: In positive testing, load is increased until an application is overwhelmed, while negative testing takes away system resources such as memory, threads or connections, until the application degrades.
...
Code architecture further impacts scalability through the implementation of fault isolation, asynchronous design, and statelessness. Fault isolative architectures, often described as "swim lanes," ensure that a failure in one component does not propagate to others, preserving the availability of the broader system. \parencite[Chapter 21]{abbott_art_2009} Additionally, asynchronous designs are more tolerant of slowdowns, as the speed of the entire system is not determined by its slowest component. This makes asynchronous systems easier to scale. \parencite[205]{abbott_art_2009} Finally, stateless systems allow requests to be distributed freely across any available server, making them more scalable. \textcite[206]{abbott_art_2009} argues that stateful applications should be avoided, when possible.
