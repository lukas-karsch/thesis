...
\label{sec:load-test-methodology}

Based on the theoretical foundations laid out in \autoref{sec:evaluating-performance-through-load-testing}, this thesis performs load testing on both implementations. The primary load parameter used is the number of concurrent requests to the web server~\cite[11]{kleppmann_designing_2017}. The tests measure \textbf{\acrfull{rps}}, and server- and client-side response times to ensure that queueing delays are accurately captured and user-perceived performance is reflected.
...
In the analysis of the results, the arithmetic mean is avoided in favor of \glspl{percentile}. The \gls{median} (P50) is used to determine the typical response time, while P95 and P99 percentiles are utilized to measure tail latency and identify the performance of outliers.
...
As described in \autoref{sec:load-testing-theory}, not just the average latency is measured, but \glspl{percentile} are used to accurately report the number of users experiencing the respective latency.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{lXl}
        ...
        \addlinespace
        $cpu\_usage$                & Process CPU usage                            & Server            \\
        \addlinespace
        ...
        \addlinespace
        $dropped\_iterations\_rate$ & Dropped iterations (per second)              & Client            \\
        \addlinespace
        $failure\_rate$             & Rate of failed requests (in \%)              & Client            \\
        \bottomrule
    \end{tabularx}
...
As outlined above, load tests are used to collect latencies and system metrics. From these results, \autoref{sec:scalability-dimensions} serves as a basis to define a scalability function specific to this thesis. Results from the scalability function can then be used to assess the scalability of each application.

\subsection{Flexibility}

\hyperref[sec:static-analysis-metrics]{Section \ref*{sec:static-analysis-metrics}} described a set of static analysis metrics. These serve as a basis to answer \hyperref[rs-architecture]{\acrshort{rq} 2}, using the results to compare the applications' architectures and their potential for flexibility and evolution over time.
...
Several qualitative criteria for the accuracy of a historic reconstruction can be defined. \emph{Source of truth integrity} defines how the system guarantees that the historic records match the current state. When a system needs to write state changes to two separate data stores, this creates a problem called "dual-write" problem in distributed systems.
...
Another interesting perspective in this scope would be \emph{schema resilience}, which examines how historical data survives the evolution of the application's data structure. As business rules change, entities and database schemas evolve. The history must remain readable without being corrupted by schema modifications. However, as \gls{schema-evolution} is not a goal of the study, this perspective will not be pursued further.
