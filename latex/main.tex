...
\usepackage{makecell}
\usepackage[bottom]{footmisc} % push footnotes to the bottom 
...
\newglossaryentry{atomicity}
{
    name=Atomicity,
    text=atomicity,
    description={means that an action is either fully executed or not at all. Atomic operations ensure the application is not left in an invalid state \cite[10]{bernstein_principles_2009}}
}
...
\vspace{1.0cm} % Space for signature
...
The full source code for this thesis, including both applications, load tests and markdown notes, is available at the following locations:
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{llX}
        \toprule
        \textbf{Kapitel / Codemodul} & \textbf{Tool(s)} & \textbf{Beschreibung \& Begründung Einsatzzweck}                                                             \\ \midrule
        ...
        \addlinespace
        Kapitel 7 (Results)          & Gemini           & Hilfe bei der Visualisierung \& statistischen Auswertung der Ergebnisse (matplotlib, pandas, seaborn, numpy) \\
        \addlinespace
        \texttt{api} Modul           & Gemini           & Generierung von Swagger-Dokumentation                                                                        \\
        \bottomrule
    \end{tabularx}
