...
Instead of starting the \acrshort{vm} directly, the script shuts the \acrshort{vm} down and converts it into a Proxmox template, which can be re-created efficiently.
...
Using k6, several test scripts were written. Show structure and example for one script: seed data in setup function, options (ramping arrival rate, open model!), export default function.

\subsection{Load Test Lifecycle}

k6 script alone is not enough: it can perform test on a running application, but thats it. To run the test and collect metrics, in between runs restart the app and dependencies, lifecycle needs to be managed.

Elements:

\begin{itemize}
    \item perf\_runner.py. Docker remote context, start docker compose, start Prometheus. After finishing test execution, collect metrics and postprocess (link to next subsection) This can maybe be described using a diagram?
    \item which relies on metric.json definition
    \item many\_runs.py to orchestrate a big amount of runs
    \item then finally, shell scripts executing the many\_runs.py many times, in between adjusting configuration of VUs (RPS)
\end{itemize}

\subsection{Post Processing Test Results}

Collected Prometheus metrics are (partly) post-processed and aggregated. For example, turn client and server latency metrics into aggregated CSV files which contain $latency\_avg$, $latency\_p50$ etc.

\subsection{Testing "Freshness": Time to Consistency (TODO)}

This is a "special" case of test because after trying to create, we want to find out how quickly that change is reflected in the read model. Implementation is still to do.
